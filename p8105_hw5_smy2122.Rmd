...
sim_results_all_mu =
  tibble(
    mu = 0:6) |>
  mutate(
    output_list = map(mu, ~rerun(5000, t_test_simulations(n = 30, mu))),
...
Next, I will make a plot showing the proportion of the number of times the null was rejected on the y-axis and the true value of mu on the x-axis.
...
sim_results_all_mu |>
  ...
  mutate(
    power = mean(p_value)) |>
  summarize(
    proportion = sum(power)/n())
```

Next, I will make two plots. The first will show the average estimate of mu-hat on the y-axis and the true value of mu on the x-axis, and the second will show the average estimate of mu-hat only in samples for which the null was rejected on the y-axis and the true value of mu on the x-axis.
